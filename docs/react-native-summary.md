...
アプリで扱うデータをデータベースのテーブルとして設計することです。画面設計で「何を表示するか」が決まったら、「そのデータをどう保存するか」を考えます。

### 外部キー（Foreign Key）

他のテーブルの `id` を参照するカラムです。テーブル同士を「つなぐ」役割を持ちます。
...
// 外部キー（どのクレカかまで特定できる）
credit_card_id: "uuid-xxxx"  ← credit_cards テーブルの id を参照
```
...
| 関係 | 例 | 設計方法 |
|------|----|---------|
| **1対多** | 1つの口座に複数のクレカ | クレカ側に `account_id` を持つ |
| **1対1** | 1つの目標に1つの口座 | どちらかに相手の `id` を持つ |
| **多対多** | タグと収支 | 中間テーブルを作る |

> **つまずきポイント：外部キーの向きを間違える**
> 「口座にクレカIDを持たせる」と1枚しか紐付けられません。
> 「多」の側（クレカ）が「1」の側（口座）の ID を持つのが正しいです。
> ```
> // 誤り：口座テーブルに credit_card_id → 1枚しか持てない
> // 正しい：クレカテーブルに account_id → 何枚でも紐付けられる
> ```

### nullable（ヌラブル）

値が入らない場合がある（`null` を許容する）カラムのことです。

```
// 例：全ての収支がクレカ払いではない
transactions.credit_card_id → nullable（現金払いのときは null）
```

### 設計のコツ

- テーブルを作るたびに `user_id` が必要か確認する（RLS との兼ね合い）
- `_flg`（フラグ）より `_id`（外部キー）の方が「どれか」まで特定できて便利
- 「誰が・何を・いつ」を意識するとカラムが見えてくる
